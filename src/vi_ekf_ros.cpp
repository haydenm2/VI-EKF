  ...
  it_(nh_)
{
  ...
  gps_sub_ = nh_.subscribe("gps", 10, &VIEKF_ROS::gps_callback, this);
...
void VIEKF_ROS::odom_truth_callback(const nav_msgs::OdometryConstPtr &msg) {
  // the rosflight_sim gazebo plugin published truth data in an odometry message
...
void VIEKF_ROS::gps_callback(const inertial_sense::GPSConstPtr &msg)
{
    if (!gps_init_)
    {
      xform::Xformd T_e_I;
    ...
    Rbe = Quatd(ekf_.get_state().segment<4>(vi_ekf::VIEKF::ATT)).R();
    ...
    double t = (msg->header.stamp - start_time_).toSec();
    ekf_mtx_.lock();
    ekf_.add_measurement(t, z_gps_, vi_ekf::VIEKF::GPS, gps_R_, !use_gps_);
    ekf_mtx_.unlock();
}
